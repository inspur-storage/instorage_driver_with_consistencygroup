...
from cinder.volume.drivers.inspur import instorage_fc
from cinder.volume.drivers.inspur import instorage_iscsi
...
class InStorageMCSManagementSimulator(object):
    ...
    def __init__(self, pool_name):
        ...
        self._system_list = {'instorage-mcs-sim': {'id': '0123456789ABCDEF',
    ...
    def _lc_cg_state_transition(self, function, lc_consistgrp):
        ...
        try:
            ...
            lc_consistgrp['status']\
                = self._lc_cg_transitions[curr_state][function]
            return ('', '')
        except Exception:
            return self._errors['CMMVC5903E']
...
class InStorageMCSISCSIFakeDriver(instorage_iscsi.InStorageMCSISCSIDriver):

    def __init__(self, *args, **kwargs):
        super(InStorageMCSISCSIFakeDriver, self).__init__(*args, **kwargs)
...
class InStorageMCSFcFakeDriver(instorage_fc.InStorageMCSFCDriver):

    def __init__(self, *args, **kwargs):
        super(InStorageMCSFcFakeDriver, self).__init__(*args, **kwargs)
...
class InStorageMCSISCSIDriverTestCase(test.TestCase):
    ...
    def setUp(self, mock_sleep):
        ...
        self.USESIM = True
        if self.USESIM:
            self.iscsi_driver = InStorageMCSISCSIFakeDriver(
            ...
            self._def_flags = {'san_ip': 'hostname',
                               ...
                               'instorage_mcs_multipath_enabled': False,
            ...
            self._connector = {'ip': '1.234.56.78',
                               ...
                               'wwpns': wwpns,
                               'initiator': initiator}
            self.sim = InStorageMCSManagementSimulator(['openstack'])
    ...
    def test_instorage_do_terminate_iscsi_connection(self, init_conn,
                                                     term_conn):
        # create a iSCSI volume
        volume_iSCSI = self._create_volume()
        extra_spec = {'capabilities:storage_protocol': '<in> iSCSI'}
    ...
    def test_instorage_terminate_iscsi_connection_multi_attach(self):
        # create a iSCSI volume
        volume_iSCSI = self._create_volume()
        extra_spec = {'capabilities:storage_protocol': '<in> iSCSI'}
        vol_type_iSCSI = volume_types.create(self.ctxt, 'iSCSI', extra_spec)
        ...
        for conn in [connector, connector2]:
            host = self.iscsi_driver._assistant.get_host_from_connector(conn)
        ...
        host_name = self.iscsi_driver._assistant.get_host_from_connector(
            connector2)
        self.iscsi_driver._assistant.unmap_vol_from_host(
    ...
    def test_instorage_mcs_iscsi_host_maps(self):
        ...
        if self.USESIM:
            ret = (
                self.iscsi_driver._assistant.get_host_from_connector(
                    self._connector))
            self.assertIsNone(ret)

    def test_instorage_mcs_iscsi_multi_host_maps(self):
        ...
        self._set_flag('instorage_mcs_multihostmap_enabled', False)
        self.assertRaises(
            exception.CinderException,
            self.iscsi_driver.initialize_connection, volume, conn2)

        self._set_flag('instorage_mcs_multihostmap_enabled', True)
        self.iscsi_driver.initialize_connection(volume, conn2)
...
class InStorageMCSFcDriverTestCase(test.TestCase):
    ...
    def setUp(self, mock_sleep):
        ...
        self.USESIM = True
        if self.USESIM:
            self.fc_driver = InStorageMCSFcFakeDriver(
                configuration=conf.Configuration(None))
            self._def_flags = {'san_ip': 'hostname',
                               'san_login': 'user',
                               'san_password': 'pass',
            ...
            self._connector = {'ip': '1.234.56.78',
                               ...
                               'wwpns': wwpns,
                               'initiator': initiator}
            self.sim = InStorageMCSManagementSimulator(MCS_POOLS)
    ...
    def test_instorage_get_host_from_connector_with_lshost_failure2(self):
        ...
        if self.USESIM:
            ...
            self.assertRaises(exception.VolumeBackendAPIException,
                              assistant.get_host_from_connector,
                              self._connector)
    ...
    def test_instorage_get_host_from_connector_not_found(self):
        ...
        if self.USESIM:
            self.sim._cmd_mkhost(
                name='instorage-mcs-test-3',
                hbawwpn='1234567')
            self.sim._cmd_mkhost(
                name='instorage-mcs-test-2',
                hbawwpn='2345678')
            self.sim._cmd_mkhost(
                name='instorage-mcs-test-1',
                hbawwpn='3456789')
    ...
    @mock.patch.object(instorage_fc.InStorageMCSFCDriver,
                       '_do_initialize_connection')
    def test_instorage_do_terminate_fc_connection(self, init_conn,
                                                  term_conn):
        # create a FC volume
        volume_fc = self._create_volume()
        extra_spec = {'capabilities:storage_protocol': '<in> FC'}
...
class InStorageMCSCommonDriverTestCase(test.TestCase):
    ...
    def setUp(self, mock_sleep):
        ...
        if self.USESIM:
            ...
            self.driver = InStorageMCSISCSIFakeDriver(
    ...
    def test_instorage_mcs_connectivity(self):
        ...
        self._set_flag('instorage_mcs_vol_rsize', 2)
        self._set_flag('instorage_mcs_vol_nofmtdisk', True)
        self.assertRaises(exception.InvalidInput,
                          self.driver.check_for_setup_error)
        self._reset_flags()
    ...
    def test_instorage_create_volume_with_replication_disable(self):
        volume = self._generate_vol_info(None, None)
    ...
    def test_instorage_create_volume_with_strech_cluster_replication(self):
        # Set replication flag, set pool openstack2 for secondary volume.
        self._set_flag('instorage_mcs_stretched_cluster_partner', 'openstack2')
    ...
    def test_instorage_create_cloned_volume_with_strech_cluster_replica(self):
        # Set replication flag, set pool openstack2 for secondary volume.
        self._set_flag('instorage_mcs_stretched_cluster_partner', 'openstack2')
        self.driver.do_setup(self.ctxt)
    ...
    def test_instorage_create_snapshot_volume_with_strech_cluster_replica(
            self):
        # Set replication flag, set pool openstack2 for secondary volume.
        self._set_flag('instorage_mcs_stretched_cluster_partner', 'openstack2')
        self.driver.do_setup(self.ctxt)
        ...
        self.assertEqual('copying', model_update['replication_status'])
        # Check the replication volume created on pool openstack2.
        attrs = self.driver._assistant.get_vdisk_attributes(vol2['name'])
        self.assertIn('openstack2', attrs['mdisk_grp_name'])

    @mock.patch('oslo_service.loopingcall.FixedIntervalLoopingCall',
                new=testutils.ZeroIntervalLoopingCall)
    def test_instorage_consistency_group_snapshot(self):
    ...
    @mock.patch('oslo_service.loopingcall.FixedIntervalLoopingCall',
                new=testutils.ZeroIntervalLoopingCall)
    def test_instorage_consistency_group_from_src_invalid(self):
    ...
    @mock.patch('oslo_service.loopingcall.FixedIntervalLoopingCall',
                new=testutils.ZeroIntervalLoopingCall)
    def test_instorage_consistency_group_from_src(self):
...
class InStorageAssistantTestCase(test.TestCase):
    ...
    def test_replication_licensed(self, lsguicapabilities):
        lsguicapabilities.side_effect = [
            {'product_key': '0000'},
            {'product_key':
                instorage_const.DEV_MODEL_INSTORAGE},
            {'product_key':
             instorage_const.DEV_MODEL_INSTORAGE_AS5X00}]
        for i in range(1):
            self.assertFalse(self.instorage_mcs_common.replication_licensed())

        for i in range(2):
            self.assertTrue(self.instorage_mcs_common.replication_licensed())
...
class InStorageSSHTestCase(test.TestCase):
    ...
    def setUp(self):
        ...
        self.fake_driver = InStorageMCSISCSIFakeDriver(
            configuration=conf.Configuration(None))
        sim = InStorageMCSManagementSimulator(['openstack'])
    ...
    def test_mkvdisk_with_warning(self, run_ssh_check, lsvol):
        ...
        with mock.patch.object(instorage_common.InStorageSSH,
            ...
            mock.patch.object(instorage_common.InStorageSSH, 'lsvdisk',
                              return_value=lsvol):
            if lsvol:
...
class InStorageMCSReplicationTestCase(test.TestCase):
    ...
    def setUp(self, mock_sleep):
        ...
        self.USESIM = True
        if self.USESIM:
            self.driver = InStorageMCSFcFakeDriver(
                configuration=conf.Configuration(None))
    ...
    def test_instorage_failback_replica_volumes(self,
        ...
        self.assertEqual(MCS_POOLS, self.driver._get_backend_pools())
        self.assertTrue(update_instorage_state.called)
        self.assertTrue(update_volume_stats.called)
        self.driver.delete_volume(mm_vol)
        self.driver.delete_volume(gm_vol)
