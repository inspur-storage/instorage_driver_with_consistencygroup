...
from cinder.volume.drivers.inspur import (
    instorage_common as instorage_common)
from cinder.zonemanager import utils as fczm_utils

LOG = logging.getLogger(__name__)

instorage_mcs_fc_opts = [
    cfg.BoolOpt('instorage_mcs_multipath_enabled',
                default=False,
...
CONF = cfg.CONF
CONF.register_opts(instorage_mcs_fc_opts)
...
class InStorageMCSFCDriver(instorage_common.InStorageMCSCommonDriver):
    ...
    def initialize_connection(self, volume, connector):
        """Perform necessary work to make a FC connection."""
        @utils.synchronized('instorage-host' + self._state['system_id'] +
    ...
    def _do_initialize_connection(self, volume, connector):
        ...
        multihostmap = self.configuration.instorage_mcs_multihostmap_enabled
        lun_id = self._assistant.map_vol_to_host(volume_name, host_name,
                                                 multihostmap)
        try:
    ...
    def terminate_connection(self, volume, connector, **kwargs):
        ...
        @utils.synchronized('instorage-host' + self._state['system_id'] + host,
                            external=True)
